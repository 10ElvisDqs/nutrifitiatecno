    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-center">
                <form action="{{route('ejercicioEquipo.update',$ejercicioEquipo)}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('put')  
                    <div class="row">
                        {{-- With prepend slot, label, and data-placeholder config --}}
                            ...
                            {{-- With prepend slot, label, and data-placeholder config --}}
                            <x-adminlte-select2 name="equipo" label="Equipo" label-class="text-lightblue" igroup-size="lg" data-placeholder="Select an option...">
                                <x-slot name="prependSlot">
                                    <div class="input-group-text bg-gradient-info">
                                        <i class="fas fa-dumbbell"></i>
                                    </div>
                                ...
                                @foreach ($equipos as $equipo)
                                    <option value="{{ $equipo->id }}" {{ isset($selectedEquipo) && $selectedEquipo->id == $equipo->id ? 'selected' : '' }}>
                                        {{ $equipo->nombre }}
                                    </option>
                                @endforeach
                            </x-adminlte-select2>                       
