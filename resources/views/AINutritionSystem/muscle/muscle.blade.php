...
@section('content')
            ...
            <x-adminlte-datatable id="table1" :heads="$heads" :config="$config">
                @foreach($muscles as $muscle)
                    <tr>
                        ...
                        <td><a href="{{route('muscle.edit',$muscle)}}"class="btn btn-xs btn-default text-primary">
                            <i class="fa fa-lg fa-fw fa-pen"></i>
                        </a>
                            <form style="display: inline" action="{{ route('muscle.destroy',$muscle)}}" method="POST"
                                class="formEliminar">
                                @csrf
                                @method('delete')
                                {!! $btnDelete !!}
    ...
    {{-- Modal --}}
    {{-- Themed --}}
    <x-adminlte-modal id="modalPurple" title="New muscle" theme="primary"
    icon="fas fa-bolt" size='lg' disable-animations>
        <div class="d-flex justify-content-center">
            <form action="{{route('muscle.store')}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="row">
